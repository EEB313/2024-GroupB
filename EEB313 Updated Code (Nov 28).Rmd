...
```{r}
dat<- as.data.frame(cbind (data, axes))
test_model1 <- lmer(formula=Shannon~PC1*PC2 + (1|Community), data=dat)
summary(test_model5)
test_model2 <- lmer(formula=Shannon~PC1+PC2 + (1|Community), data=dat)
summary(test_model6)
test_model3 <- lmer(formula=Shannon~PC1 + (1|Community), data=dat)
summary(test_model7)
test_model4 <- lmer(formula=Shannon~PC2 + (1|Community), data=dat)
summary(test_model8)
...
```{r}
dat  |>  
  mutate(fit.m = predict(test_model7, re.form = NA), # does not include random effects
         fit.c = predict(test_model7, re.form = NULL) # includes random effects
         ) ->
  predicted_values 
...
```{r}
dat  |>  
  mutate(fit.m = predict(test_model8, re.form = NA), # does not include random effects
         fit.c = predict(test_model8, re.form = NULL) # includes random effects
         ) ->
  predicted_values 
...
predicted_values |>
  ...
  geom_point(size = 3) +
  geom_line(inherit.aes = F, aes(x = PC2, y = fit.c, color = Community), size = 1) +
  geom_line(inherit.aes = F, aes(x = PC2, y = fit.m), color = "black", size = 2)
```
